...
原需求把「管理后台 UI」列在 Out of scope 里，停止线也不含前端。但实际打开项目时，
能看到的只有 Swagger——那是开发者工具，不是能给人看的东西。对一个要拿去展示的项目，
这是个真实的缺口。

### 关键决策

**静态 HTML，由 FastAPI 自己托管，不引入任何前端框架和构建工具。**
理由是项目原则本身（§3.3「不为了技术栈丰富引入」、§5.1「保持单体」）：加一套
React + npm + 打包，会让这个项目多出一整个需要解释的技术栈，而它对"并发安全"这个
核心主张毫无贡献。

**演示的重点不是转盘，是让并发特性可以被当场证伪。** 页面主体是一个「并发测试」面板：
填 N，同时打出 N 个请求，跑完直接对账——产生的订单数是否等于消耗的库存、库存有没有
被扣成负数、勾上「同一 request_id」后是否只产生一个订单。

一个漂亮的转盘只能证明接口能调通；这个面板能让人现场尝试推翻项目的核心主张。
对一个以"每条技术主张都有证据"为原则的项目，后者才是对的演示方式。

**范围文档同步修正。** 没有偷偷加东西：REQUIREMENTS §1.3 的 Out of scope 里
明确加了这条例外，说明它是演示页而非运营后台。

### 踩的坑（两个都是我自己的）

**1. `replace` 静默失败。** 给 `main.py` 加 `StaticFiles` 导入时，我假设的导入块顺序
和文件里的实际顺序不一致（中间插了之前加的 `SATimeoutError`），`s.replace()` 什么也
没做就返回了——**这一处我没写 assert**。结果是容器构建成功、启动失败，
`NameError: name 'StaticFiles' is not defined`。

更关键的是我**改完没有本地验证就直接构建镜像**。前面几次改动我都会先跑一次
`import app.main`，这次跳过了，白白多花一轮构建。

**2. 页面自己的校验逻辑写错了。** 勾上「同一 request_id」跑并发时，页面报
"不变量被破坏"：
...
系统是对的（幂等那条 ✓ 通过了，1 个订单 1 个库存），错的是我的统计口径——
91 个请求拿到的是**回放的结果**，`order_id` 全都相同，它们不是新的抽奖。
应该数**去重后的订单数**而不是成功响应数。

这个 bug 的危害不在于难修，而在于**它会让人误以为系统坏了**。一个用来证明正确性的
工具，自己出错时的破坏力比普通 bug 大——这和 Phase 4 那四个压测工具的坑是同一类问题。

### 补做：转盘

第一版只做了工程验证面板，没有转盘。但真正面向用户的抽奖产品，转盘就是主界面——
工程面板是给我自己和面试官看的，转盘才是给用户看的。两者用途不同，都需要。

**转盘必须停在后端返回的那个奖品上。** 前端不能自己再随机一次，否则转盘显示的结果和
真实落库的订单会对不上。实现上是：先调接口拿到结果，再计算该转到哪个角度。
...
扇区面积按真实权重分配，所以"谢谢参与"占了大半圈——这本身就是个诚实的展示：
用户一眼能看出中奖概率有多低。

**被拒绝时不转。** 限流、配额用完、无库存这些，转完 4 秒再告诉用户"你被限流了"很莫名
其妙。直接抖一下弹出原因。

### 再改：视觉重做成消费级

第一版转盘是深色开发者审美，被指出"太简陋、不像真实产品"。重做成国内营销活动的
通用视觉语言：红金渐变背景、金边灯珠转盘、红金交替扇区、圆角奖品卡、中奖弹窗、
滚动中奖播报。

**播报只播真实发生过的抽奖**（本次会话里在这个页面抽出来的），不编造假数据。
造一堆"用户***刚刚中了iPhone"很容易，但那和这个项目"每条主张都有证据"的原则相悖。

**工程面板保留，但降为次级区块**，标题直接叫「开发者面板」。两块东西面向不同的人：
转盘给用户看，面板给我自己和面试官看，谁也替代不了谁。

踩了两个 Canvas 画转盘的经典坑：

1. **大扇区的文字贴边就读不出来。** "谢谢参与"占 70%，文字按 `textAlign=right` 贴着
   圆周排，几乎糊成一团。改成按扇区居中、大扇区用更大字号并往圆心方向内移。
2. **扇区中心角落在 90°–270° 时文字上下颠倒。** 跟随旋转的文字在下半圈会倒过来。
   标准解法是再转 180° 并把文字放到反方向：

   ```js
   if (mid > 90 && mid < 270) { ctx.rotate(Math.PI); tx = -tx; }
   ```

两个都是肉眼一看就发现的问题——但只有真的在浏览器里渲染出来才看得见，
靠读代码发现不了。

### 再改：转盘换成九宫格

按要求改成 PDD 那种 3×3 九宫格，点击后高亮在外圈 8 格之间跳动、缓出停在中奖格。

**格子数必须视觉平均，不能按权重分配。** 我第一反应是"格子多少反映概率，用户一眼看得
出来"——这个想法是错的。真实的营销抽奖不会把中奖率摆在脸上，而且一旦格子数携带概率
信息，用户数一下就能反推出来。4 个奖品各占 2 格，轮流铺开，格子数不传递任何信息，
概率只存在于后端的 `weight` 里。

顺带解决了转盘方案的一个固有缺陷：转盘的扇区面积**必然**暴露概率（"谢谢参与"占 70%
就是大半圈，藏都藏不住）。九宫格没这个问题。

**落点仍然由后端决定。** 同一奖品占了 2 格，落哪一格都合法，随机挑一个即可；
但落在哪个**奖品**上，前端无权决定。

### 又一次栽在转义折叠上

改完发现页面整个白屏，控制台 `Uncaught SyntaxError: Invalid or unexpected token`。
原因还是那个——我在生成脚本里写的 `
`，到 Python 之前被折叠成了**真实换行**，
于是 JS 里出现了跨行的双引号字符串：
...
这是同一个根因在这个项目里的第**六**次出现（requirements.txt、alembic.ini、
pytest.ini、subprocess 解码、打补丁的锚点、这次）。前五次我都是"发现了、修掉、记下来"，
但没有改变做法。

这次改成从根上回避：需要多行文本就用数组 `join(String.fromCharCode(10))`，
源码里一个转义序列都不写。**反复踩同一个坑，说明要改的不是警惕程度，而是做法本身。**

### 拆成两个页面

原本抽奖界面和开发者面板挤在同一页，而且奖品卡上还挂着概率角标——这和"九宫格刻意做成
平均格子数"自相矛盾：一边藏概率，一边把它印在下面。

拆开：`/` 是纯用户页（九宫格、弹窗、播报、稀缺提示，**零概率信息**），
`/dev` 是开发者页（真实 weight、归一化中奖率、请求响应原文、并发验证）。
公共工具抽到 `common.js`。

**加了一条测试盯着这个边界**：用户页的 HTML 与 JS 里不得出现 `class="rate"`、
不得用 `weight` 做渲染、HTML 里不得出现 `%`。哪天有人手滑把概率加回用户页，
这条会挡住。

写这条测试时先写错了一版——把注释也算进去，于是 JS 里"为什么不展示中奖率"的**注释**
触发了断言。注释里解释设计意图是应该的，会泄露的是**渲染出来的内容和渲染它的代码**。
断言的对象搞错了，测的就不是想测的东西。

顺带把用户身份改成 localStorage 持久化的匿名 ID：项目没有账号体系
（见 Known Limitations），但至少要让"每人每日 N 次"这条规则对真实用户有意义，
而不是每次刷新就重置。
