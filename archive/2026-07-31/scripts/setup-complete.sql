...
-- ============================================
-- 使用方法：
-- 1. 在 DataGrip 或其他 MySQL 客户端执行此脚本
-- 2. 或者在命令行执行：mysql -u root -p [REDACTED_LOCAL_CREDENTIAL] setup-complete.sql
-- ============================================
...
CREATE TABLE IF NOT EXISTS `activity` (
    ...
    `state` tinyint(2) NOT NULL DEFAULT '1' COMMENT '活动状态：1编辑、2提审、3撤审、4通过、5运行(审核通过且worker扫描状态)、6拒绝、7关闭、8开启',
...
CREATE TABLE IF NOT EXISTS `strategy` (
    ...
    `strategy_mode` tinyint(2) NOT NULL DEFAULT '1' COMMENT '策略方式：1单项概率、2总体概率',
    `grant_type` tinyint(2) NOT NULL DEFAULT '1' COMMENT '发放方式：1即时、2定时[含活动结束]、3人工',
...
CREATE TABLE IF NOT EXISTS `award` (
    `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增ID',
    `award_id` varchar(64) NOT NULL COMMENT '奖品ID',
    `award_type` tinyint(2) NOT NULL COMMENT '奖品类型：1文字描述、2兑换码、3优惠券、4实物奖品',
